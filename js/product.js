// PRODUCT DETAIL — reads ?id= from the URL, fills the page from PRODUCTS.

const params = new URLSearchParams(window.location.search);
const id = params.get('id');
const product = PRODUCTS.find(p => p.id === id) || PRODUCTS[0];

// Fill in details
document.title = `${product.title} — SUMMERTONEZ`;
document.getElementById('pdpTitle').innerHTML = product.title;
document.getElementById('pdpPrice').textContent = `$${product.price}`;
document.getElementById('pdpDesc').textContent = product.desc;
document.getElementById('pdpCrumb').textContent = product.title;

// Main image — `images` (multiple angles) takes priority over the single `image`.
const gallery = (product.images && product.images.length) ? product.images : (product.image ? [product.image] : []);
const mainEl = document.getElementById('pdpMain');
if (gallery.length) {
  mainEl.innerHTML = `<img src="${gallery[0]}" alt="${product.title}" />`;
} else {
  mainEl.innerHTML = `<image-slot id="pdp-main-slot-${product.id}" placeholder="${product.title}" shape="rect"></image-slot>`;
}

// Thumbnails — one per photo in the gallery.
const thumbsEl = document.getElementById('pdpThumbs');
const thumbs = [];
if (gallery.length) {
  gallery.forEach(src => {
    thumbs.push({html: `<img src="${src}" alt="" />`, main: `<img src="${src}" alt="${product.title}" />`});
  });
} else {
  thumbs.push({html: `<image-slot id="thumb-0-${product.id}" placeholder="Front" shape="rect"></image-slot>`, main: `<image-slot id="pdp-main-slot-${product.id}" placeholder="${product.title}" shape="rect"></image-slot>`});
}
thumbsEl.innerHTML = thumbs.map((t, i) => `<div class="pdp-thumb${i===0?' active':''}" data-idx="${i}">${t.html}</div>`).join('');
thumbsEl.querySelectorAll('.pdp-thumb').forEach((el, i) => {
  el.addEventListener('click', () => {
    thumbsEl.querySelectorAll('.pdp-thumb').forEach(x => x.classList.remove('active'));
    el.classList.add('active');
    mainEl.innerHTML = thumbs[i].main;
  });
});

// Sizes
const sizesEl = document.getElementById('pdpSizes');
const sizeChoice = document.getElementById('pdpSizeChoice');
let selectedSize = null;
const sizes = product.sizes || ['S', 'M', 'L', 'XL'];
const soldSizes = product.soldSizes || [];
sizesEl.innerHTML = sizes.map(s => {
  const sold = soldSizes.includes(s);
  return `<button class="size-btn" data-size="${s}"${sold ? ' disabled' : ''}>${s}</button>`;
}).join('');
sizesEl.querySelectorAll('.size-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    if (btn.disabled) return;
    sizesEl.querySelectorAll('.size-btn').forEach(x => x.classList.remove('selected'));
    btn.classList.add('selected');
    selectedSize = btn.dataset.size;
    sizeChoice.textContent = `— ${selectedSize}`;
    updateAddBtn();
  });
});

// Quantity
const qtyInput = document.getElementById('qtyInput');
document.getElementById('qtyMinus').addEventListener('click', () => {
  qtyInput.value = Math.max(1, parseInt(qtyInput.value || 1) - 1);
});
document.getElementById('qtyPlus').addEventListener('click', () => {
  qtyInput.value = Math.min(10, parseInt(qtyInput.value || 1) + 1);
});

// Drop countdown — while a release is pending, "Add to bag" stays locked
// and a live clock counts down to it. Presentation only; Shopify is the
// real gate (see js/drop.js).
const dropEl = document.getElementById('pdpDrop');
let dropPending = isUpcoming(product);

function renderDrop() {
  const left = dropRemaining(product);
  if (!left) {
    // The moment has arrived — unlock without needing a reload.
    dropEl.hidden = true;
    if (dropPending) {
      dropPending = false;
      updateAddBtn();
    }
    return;
  }
  const pad = (n) => String(n).padStart(2, '0');
  dropEl.hidden = false;
  dropEl.innerHTML = `
    <div class="pdp-drop-label"><span class="hindi">ਜਲਦੀ ਆ ਰਿਹਾ</span> · drops ${dropLabel(product)}</div>
    <div class="pdp-drop-clock">
      <span><b>${pad(left.days)}</b>days</span>
      <span><b>${pad(left.hours)}</b>hrs</span>
      <span><b>${pad(left.mins)}</b>min</span>
      <span><b>${pad(left.secs)}</b>sec</span>
    </div>`;
}

if (product.dropsAt) {
  renderDrop();
  setInterval(renderDrop, 1000);
}

// Add to bag
const addBtn = document.getElementById('addBtn');
function updateAddBtn() {
  if (dropPending) {
    addBtn.disabled = true;
    addBtn.textContent = `Drops ${dropLabelShort(product)}`;
  } else if (product.sold) {
    addBtn.disabled = true;
    addBtn.textContent = 'Sold out';
  } else if (!selectedSize) {
    addBtn.disabled = false;
    addBtn.textContent = 'Select size';
  } else {
    addBtn.disabled = false;
    addBtn.textContent = 'Add to bag →';
  }
}
updateAddBtn();
addBtn.addEventListener('click', async () => {
  if (dropPending) return;
  if (!selectedSize && !product.sold) {
    addBtn.textContent = 'Choose a size first';
    setTimeout(updateAddBtn, 1600);
    return;
  }
  const variantId = product.variants && product.variants[selectedSize];
  if (!variantId) {
    addBtn.textContent = 'Not available yet';
    setTimeout(updateAddBtn, 1800);
    return;
  }
  addBtn.disabled = true;
  addBtn.textContent = 'Adding…';
  try {
    const cart = await addToCart(variantId, parseInt(qtyInput.value));
    updateBagLinks(cart);
    addBtn.classList.add('added');
    addBtn.textContent = '✓ Added';
  } catch (err) {
    addBtn.textContent = 'Something went wrong';
  }
  setTimeout(() => {
    addBtn.classList.remove('added');
    addBtn.disabled = false;
    updateAddBtn();
  }, 1800);
});

// Related — other products, same simple-product card as the shop page
const related = PRODUCTS.filter(p => p.id !== product.id).slice(0, 3);
document.getElementById('relatedGrid').innerHTML = related.map(p => `
  <a class="simple-product" href="/product?id=${p.id}">
    <div class="simple-product-media">
      ${p.image
        ? `<img src="${p.image}" alt="${p.title}" class="simple-product-img" />`
        : `<image-slot id="rel-${p.id}" placeholder="${p.title}" shape="rect" class="simple-product-img"></image-slot>`
      }
    </div>
    <div class="simple-product-info">
      <div class="simple-product-row">
        <span class="simple-product-title">${p.title}</span>
        <span class="simple-product-price">$${p.price}</span>
      </div>
      ${p.punjabi ? `<div class="simple-product-punjabi">${p.punjabi}</div>` : ''}
    </div>
  </a>
`).join('');
